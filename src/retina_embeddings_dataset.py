...
def _derive_tasks_mbrset(labels: pd.DataFrame) -> pd.DataFrame:
    ...
    # Clinical tasks
    # IMPORTANT: Preserve NaN from dr_grade — images with unknown DR status must
    # remain NaN (not be silently mapped to 0/negative).  The old code
    #   (dr_grade >= 1).astype("Int64")
    # converts NaN → False → 0, contaminating the negative class.
    labels["task_any_dr"] = labels["dr_grade"].apply(
        lambda x: 1 if x >= 1 else (0 if x == 0 else pd.NA)
    ).astype("Int64")
    def _referable(dr, edema):
        dr_pos = (dr >= 2) if pd.notna(dr) else None
        ed_pos = (edema == 1) if pd.notna(edema) else None
        # If either criterion is definitely True → referable
        if dr_pos is True or ed_pos is True:
            return 1
        # If both are definitely False → not referable
        if dr_pos is False and ed_pos is False:
            return 0
        # If one is False and other unknown → unknown
        if dr_pos is False and ed_pos is None:
            return 0  # DR confirmed low; missing edema treated as absent
        return pd.NA
    labels["task_referable"] = labels.apply(
        lambda r: _referable(r["dr_grade"], r["edema_bin"]), axis=1
    ).astype("Int64")
...
def _derive_tasks_brset(labels: pd.DataFrame) -> pd.DataFrame:
    ...
    def _referable_brset(dr, edema):
        ...
        return pd.NA
    labels["task_referable"] = labels.apply(
        lambda r: _referable_brset(r["dr_grade"], r["edema_bin"]), axis=1
    ).astype("Int64")
